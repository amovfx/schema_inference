...
class SchemaInferenceParser(JsonOutputParser):
    ...
    @staticmethod
    def create_infered_json_parser(infered_model_dict: dict) -> Type[BaseModel]:
        """
        param meta: the dict of the SchemaInferenceModel. This is generated by an llm.
        """
        infered_model = SchemaInferenceModel(**infered_model_dict)
        fields = {
            field.name: (
                type_map.get(field.type, str),
                Field(default=..., description=field.description),
            )
            for field in infered_model.fields
        }
        dynamic_pydantic_model = create_model(infered_model.name, **fields)
